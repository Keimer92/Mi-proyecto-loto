...
def crear_tabla():
   ...
   # Insertar una clave inicial si no existe
    ...
    if not cursor.fetchone():
        ...
        messagebox.showinfo("Configuración Inicial", "Se ha establecido 12345 como clave por defecto. Por favor, cámbiela después de iniciar sesión.")
...
class AppLoteria:
    ...
    def cambiar_tema_azure(self):
        nuevo_tema = self.tema_actual_var.get()
        if nuevo_tema in self.style.theme_names():
            try:
                # Solo cargar azure.tcl si se elige un tema azure y aún no está cargado
                if "azure" in nuevo_tema and nuevo_tema not in self.style.theme_names():
                ...
                actualizar_tema_db(nuevo_tema)
                messagebox.showinfo("Tema Aplicado", f"Tema '{nuevo_tema}' aplicado correctamente.")
            except Exception as e:
    ...
    def crear_widgets_tab_configuracion(self):
        ...
        self.btn_aplicar_tema = ttk.Button(
            self.frame_selector_tema,
            text="Aplicar Tema",
            command=self.cambiar_tema_azure,
