...
class AppLoteria:
    ...
    def crear_widgets_tab_ventas(self):
        self.tab_ventas.rowconfigure(0, weight=1)
        self.tab_ventas.columnconfigure(0, weight=1)
        self.tab_ventas.columnconfigure(1, weight=1)
        self.tab_ventas.columnconfigure(2, weight=1)

        # --- Panel de venta de número ---
        self.frame_venta = ttk.LabelFrame(self.tab_ventas, text="Vender Número")
        self.frame_venta.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")
        ...
        self.frame_estado.grid(row=1, column=0, columnspan=3, padx=15, pady=5, sticky="ew")
        ...
        # --- Historial del número (buscador y resumen) ---
        self.frame_busqueda = ttk.LabelFrame(self.tab_ventas, text="Historial del Número")
        self.frame_busqueda.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")
        self.frame_busqueda.columnconfigure(0, weight=1)
        self.frame_busqueda.rowconfigure(3, weight=1)

        # Subframe para entrada + botón
        self.frame_busqueda_input = ttk.Frame(self.frame_busqueda)
        self.frame_busqueda_input.grid(row=0, column=0, sticky="w", padx=5, pady=(5, 2))

        
        ttk.Label(self.frame_busqueda, text="Número:").grid(row=0, column=0, padx=5, pady=(5, 2), sticky="w")
        self.numero_busqueda_var = tk.StringVar()
        self.entry_numero_busqueda = ttk.Entry(self.frame_busqueda, textvariable=self.numero_busqueda_var, width=6)
        self.entry_numero_busqueda.grid(row=1, column=0, padx=5, pady=2, sticky="w")
        self.entry_numero_busqueda.bind("<Return>", lambda event: self.actualizar_historial_treeview())

        self.btn_buscar_historial = ttk.Button(self.frame_busqueda, text="Buscar", command=self.actualizar_historial_treeview)
        self.btn_buscar_historial.grid(row=2, column=0, padx=5, pady=5)

        self.tree_historial = ttk.Treeview(self.frame_busqueda, columns=("Valor",), show="headings", height=7)
        self.tree_historial.grid(row=3, column=0, padx=5, pady=5, sticky="nsew")
        self.tree_historial.heading("Valor", text="Resumen")

        # --- Ganadores del día ---
        self.frame_ganadores_dia = ttk.LabelFrame(self.tab_ventas, text="Ganadores del Día")
        self.frame_ganadores_dia.grid(row=0, column=2, padx=10, pady=10, sticky="nsew")
    ...
    def vender_numero_gui(self):
        ...
        try:
            ...
            if "múltiplo de 5" in mensaje or "Cantidad inválida" in mensaje:
                self.apuesta_var.set("")  # 🧽 Limpia el campo
