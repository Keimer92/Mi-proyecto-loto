...
class AppLoteria:
    def __init__(self, root):
        ...
        self.mes_resumen_var = tk.StringVar(value=datetime.now().strftime('%B').capitalize())
    ...
    def actualizar_resumen_ventas_dia(self):
        ...
        if periodo == "diario":
            ...
            mes = mes_map.get(self.mes_resumen_var.get(), "01")
    ...
    def crear_widgets_tab_ventas(self):
        ...
        self.frame_filtros_resumen = ttk.LabelFrame(self.frame_venta, text="Filtros de Resumen Diario")
        self.frame_filtros_resumen.grid(row=6, column=0, columnspan=2, sticky="ew", padx=5, pady=10)

        # Filtro por periodo
        ttk.Label(self.frame_filtros_resumen, text="Periodo:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        ttk.Combobox(
        ...
        # Filtro por sorteo
        ttk.Label(self.frame_filtros_resumen, text="Sorteo:").grid(row=1, column=0, padx=5, pady=5, sticky="w")
        ttk.Combobox(
            self.frame_filtros_resumen,
        ...
        self.lbl_mes_resumen.grid(row=2, column=0, ...)
        self.combo_mes_resumen.grid(row=2, column=1, ...)
        self.lbl_anio_resumen.grid(row=3, column=0, ...)
        self.combo_anio_resumen.grid(row=3, column=1, ...)

        self.lbl_fecha_ini_resumen.grid(row=4, column=0, ...)
        self.fecha_ini_entry_resumen.grid(row=4, column=1, ...)
        self.lbl_fecha_fin_resumen.grid(row=5, column=0, ...)
        self.fecha_fin_entry_resumen.grid(row=5, column=1, ...)


        self.combo_mes_resumen = ttk.Combobox(self.frame_filtros_resumen, textvariable=self.mes_resumen_var,
        ...
        ttk.Label(self.frame_filtros_resumen, text="Año:").grid(row=3, column=0, padx=5, pady=5, sticky="w")
        self.combo_anio_resumen = ttk.Combobox(self.frame_filtros_resumen, textvariable=self.anio_resumen_var,
        ...
        ttk.Label(self.frame_filtros_resumen, text="Desde:").grid(row=4, column=0, padx=5, pady=5, sticky="w")
        self.fecha_ini_entry_resumen = DateEntry(self.frame_filtros_resumen, textvariable=self.fecha_ini_resumen_var, date_pattern='yyyy-mm-dd', width=12)
        self.fecha_ini_entry_resumen.grid(row=4, column=1, padx=5, pady=5, sticky="w")

        ttk.Label(self.frame_filtros_resumen, text="Hasta:").grid(row=5, column=0, padx=5, pady=5, sticky="w")
        self.fecha_fin_entry_resumen = DateEntry(self.frame_filtros_resumen, textvariable=self.fecha_fin_resumen_var, date_pattern='yyyy-mm-dd', width=12)
