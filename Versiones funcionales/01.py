...
def crear_tabla():
    ...
    if not cursor.fetchone():
        clave_inicial_hash = hashlib.sha256("12345".encode()).hexdigest()
        cursor.execute("INSERT INTO configuracion (clave, valor) VALUES (?, ?)", ('clave_acceso', clave_inicial_hash))
        messagebox.showinfo("Configuración Inicial", "Se ha establecido la clave de acceso inicial: '12345'. Por favor, cámbiela después de iniciar sesión.")
...
def obtener_ventas_para_reporte_db(tipo_reporte=None, fecha_seleccionada=None, mes_numero_seleccionado=None, anio_seleccionado=None, sorteo_seleccionado=None):
    """
    ...
    if tipo_reporte:
        if tipo_reporte == 'diario':
            ...
            fecha_inicio = ahora - timedelta(days=dias_restar)
            ...
            if mes_numero_seleccionado and anio_seleccionado:
                try:
                    ...
                    pass
        elif tipo_reporte == 'por_fecha': # Mantener para la selección de fecha específica
            if fecha_seleccionada:
                where_clauses.append("venta_fecha_solo_dia = ?")
...
def obtener_ganadores_para_reporte_db(tipo_reporte=None, fecha_seleccionada=None, mes_numero_seleccionado=None, anio_seleccionado=None, sorteo_seleccionado=None):
    """
    ...
    if tipo_reporte:
        if tipo_reporte == 'diario':
            ...
            params.append(fecha_inicio.strftime('%Y-%m-%d')) # Solo fecha
        elif tipo_reporte == 'semanal':
            dias_restar = ahora.weekday() # 0 para lunes, 6 para domingo
            fecha_inicio = ahora - timedelta(days=dias_restar)
            fecha_inicio = fecha_inicio.replace(hour=0, minute=0, second=0, microsecond=0)
...
class AppLoteria:
    def __init__(self, root):
        ...
        self.style.theme_use('clam') # Un tema más neutral y personalizable
    ...
    def crear_widgets_tab_configuracion(self):
        ...
        self.btn_cambiar_clave = ttk.Button(self.frame_acciones, text="Cambiar Clave de Acceso", command=self.mostrar_ventana_cambiar_clave)
    ...
    def crear_widgets_tab_reportes(self):
        ...
        ttk.Radiobutton(self.frame_controles_reporte, text="Por Fecha", variable=self.report_type, value="por_fecha", command=self.on_report_type_change).grid(row=1, column=4, padx=5, pady=5, sticky="w") 

        # --- Widgets para selección de fecha específica (DateEntry) ---
        self.lbl_fecha_especifica = ttk.Label(self.frame_controles_reporte, text="Fecha Específica:")
        self.date_entry_reporte = DateEntry(self.frame_controles_reporte, width=12, background='darkblue',
                                             foreground='white', borderwidth=2, date_pattern='yyyy-mm-dd',
    ...
    def on_report_type_change(self):
        ...
        # Ocultar todos los widgets de fecha/mes/año específicos primero
        self.lbl_fecha_especifica.grid_remove()
        self.date_entry_reporte.grid_remove()
        ...
        if report_type == "por_fecha":
            self.lbl_fecha_especifica.grid(row=current_row, column=0, padx=5, pady=5, sticky="w")
            self.date_entry_reporte.grid(row=current_row, column=1, columnspan=4, padx=5, pady=5, sticky="ew")
            current_row += 1 
    ...
    def generar_reporte_gui(self):
        ...
        if tipo_periodo == "por_fecha":
            try:
                fecha_seleccionada = self.date_entry_reporte.get_date().strftime('%Y-%m-%d')
            except ValueError:
        ...
        if tipo_datos == "Ventas":
            # Aquí, la función `obtener_ventas_para_reporte_db` es la que ya agrupa y ordena
        ...
        titulo_map = {
            ...
            "por_fecha": f"para {fecha_seleccionada}"
        }
    ...
    def imprimir_reporte_gui(self):
        ...
        try:
            ...
            if sys.platform == "win32":
                ...
                subprocess.Popen(["open", "-a", "Preview", temp_pdf_path]) 
            else:
