...
class AppLoteria:
    ...
    def crear_widgets_tab_sorteos(self):
        ...
        #---- "Botón Consultar desde web" ----
        ttk.Button(
            registrar,
            text="Consultar desde Web",
            command=self.consultar_resultados_web,
            style="Naranja.TButton"
        ).grid(row=4, column=0, columnspan=2, pady=5, sticky="ew")


        # ── Etiqueta de estado: faltan o ya están todos los sorteos ──
        self.lbl_estado_sorteos = ttk.Label(registrar, text="", font=("Arial", 10, "bold"))
        self.lbl_estado_sorteos.grid(row=5, column=0, columnspan=2, pady=5, sticky="w")
    ...
    def consultar_resultados_web(self):
        fecha = self.date_entry_registro.get_date().strftime('%Y-%m-%d')
        web = obtener_resultados_loto_nicaragua()
        if not web:
            messagebox.showwarning("Web Loto", "No se obtuvo ningún dato.")
            return

        encontrados = list(web.keys())
        registrados, ya, no_vistos = [], [], []

        for sorteo in ["11 AM", "03 PM", "09 PM"] + (["06 PM"] if datetime.now().weekday()==5 else []):
            num_web = web.get(sorteo)
            num_loc = consultar_numero_ganador_db(fecha, sorteo)

            if num_web:
                if num_loc:
                    ya.append(f"{sorteo}: {num_loc}")
                else:
                    ex, _ = registrar_numero_ganador_db(fecha, sorteo, num_web)
                    if ex: registrados.append(f"{sorteo}: {num_web}")
            else:
                no_vistos.append(sorteo)

        msg = (
            f"📅 Fecha: {fecha}\n"
            f"🔍 Encontrados: {len(encontrados)} sorteos\n"
            f"✅ Nuevos registros: {len(registrados)}\n"
            f"📌 Ya existían: {len(ya)}\n"
    ...
    def verificar_estado_sorteos_del_dia(self):
        """Marca en rojo o verde según qué sorteos de Diaria faltan por ingresar hoy."""
        fecha = self.date_entry_registro.get_date().strftime('%Y-%m-%d')
        from datetime import datetime
        es_sabado = datetime.now().weekday() == 5

        # Lista de sorteos válidos para Diaria hoy
        sorteos_hoy = ["11 AM", "03 PM", "09 PM"] + (["06 PM"] if es_sabado else [])
        pendientes = []

        for s in sorteos_hoy:
            if not consultar_numero_ganador_db(fecha, s):
                pendientes.append(s)

        if pendientes:
            texto = f"⚠️ Faltan: {', '.join(pendientes)}"
            self.lbl_estado_sorteos.config(text=texto, foreground="red")
        else:
            self.lbl_estado_sorteos.config(text="✅ Todos los sorteos están registrados", foreground="green")
